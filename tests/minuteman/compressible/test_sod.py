...
def test_contact_surface_speed():
    ...
    gam=1.4
    a1 = (gam*287*T)**0.5
    assert sod.contact_surface_speed(pratio, a1, gam) == pytest.approx(756.0737668928916)
...
def test_moving_shock_temperature_ratio():
    ...
    pratio=10
    gam=1.4
    assert sod.moving_shock_temperature_ratio(pratio, gam) == pytest.approx(2.622950819672131)
...
def test_velocity_expansion_fan():
    actual = sod.velocity_expansion_fan(a4=300, x=np.array([1,2,3]), t=0.1, gam=1.4)
    np.testing.assert_allclose(actual, np.array([258.33333333, 266.66666667, 275.]))

def test_sound_speed_expansion_fan():
    actual = sod.sound_speed_expansion_fan(a4=205, u=np.array([1000,2000,30]), gam=1.43)
